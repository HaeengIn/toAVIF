...
def normalize_target_size(
    width: int | None, height: int | None, img_width: int, img_height: int
) -> tuple[int, int]:
    if width and height:
        return width, height
...
async def convert_images(
    ...
    if len(files) > MAX_FILES:
        raise HTTPException(
            status_code=400,
            detail=f"최대 {MAX_FILES}개 파일까지만 업로드할 수 있습니다.",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="설정 값이 올바르지 않습니다."
        ) from exc
    ...
    for index, file in enumerate(files):
        ...
        raw = await file.read()
        source_path = (
            batch_upload_dir / f'{index}_{Path(file.filename or "image").name}'
        )
    ...
    if len(converted) >= 2:
        ...
        with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zipf:
            for item in converted:
                zipf.write(
                    batch_output_dir / item["converted_name"],
                    arcname=item["converted_name"],
                )
        zip_url = f"/api/download-zip/{conversion_id}"

    return JSONResponse(
        {"conversion_id": conversion_id, "converted": converted, "zip_url": zip_url}
    )
...
async def download_file(conversion_id: str, filename: str):
    ...
    if not file_path.exists():
        raise HTTPException(
            status_code=404, detail="파일이 존재하지 않거나 만료되었습니다."
        )
...
async def download_zip(conversion_id: str):
    ...
    if not zip_path.exists():
        raise HTTPException(
            status_code=404, detail="ZIP 파일이 존재하지 않거나 만료되었습니다."
        )
    return FileResponse(
        zip_path, media_type="application/zip", filename=f"{conversion_id}.zip"
    )
