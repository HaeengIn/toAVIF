...
const setProgress = (upload = 0, convert = 0, download = 0) => {
  ...
  state.files.forEach((file, idx) => {
    ...
    wrap.innerHTML = `<div>${file.name}</div>
      <div class='preview-controls'>
        ...
        <label class='checkbox-field'><input type='checkbox' data-key='remove_metadata' data-idx='${idx}' ${settings.remove_metadata ? 'checked' : ''}> <span>메타데이터 삭제</span></label>
      </div>`;
...
function applyGlobalSettings() {
  ...
  const quality = document.getElementById('globalQuality').value || defaultQuality;
...
['globalWidth', 'globalHeight', 'globalQuality', 'globalRemoveMetadata'].forEach((id) => {
